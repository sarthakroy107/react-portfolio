    ...
    return(
        ...
        <AnimateSvg reference={ref}/>
            <motion.div transition={{duration: 0.8, type: "spring"}} initial={{y:50, opacity:0 }} whileInView={{y:0, opacity: 1}} >
                <h3 className='font-bold text-2xl'>{degree} <span className='text-primaryDark'>{name}</span></h3>
                <span className='text-light/75 '>{year}&nbsp; |&nbsp; </span>
                <span className='text-light/75 '>{location}</span>
                <p className='font-semibold text-xl'>CGPA: {gpa}</p>
            </motion.div>
        </div>
    )
}
    ...
    <main className=' h-screen flex flex-col justify-evenly'>
        ...
        <div className=''>
            ...
            <div className='relative w-full' ref={ref}>
                <motion.div style={{scaleY: scrollYProgress}}  className='absolute left-1/3 h-[90%] w-[4px] bg-primaryDark origin-top'/>

                <ul className='flex flex-col text-light items-center justify-evenly ml-8'>
                    <div>
                        <Details name="@JGEC" degree="B.Tech" year="2021-2025" location="Jalpaiguri, India" gpa="8.5" />
